 ...
 * @return void
 */
function cenote_woocommerce_scripts() {
	wp_enqueue_style( 'cenote-woocommerce-style', get_template_directory_uri() . '/woocommerce.css' );
	...
	wp_add_inline_style( 'cenote-woocommerce-style', $inline_font );
}
add_action( 'wp_enqueue_scripts', 'cenote_woocommerce_scripts' );
 ...
 *
	<?php
		if ( function_exists( 'cenote_woocommerce_header_cart' ) ) {
			cenote_woocommerce_header_cart();
		}
	?>
 */

if ( ! function_exists( 'cenote_woocommerce_cart_link_fragment' ) ) {
	/**
	 * Cart Fragments.
	 *
	 ...
	 */
	function cenote_woocommerce_cart_link_fragment( $fragments ) {
		ob_start();
		cenote_woocommerce_cart_link();
		$fragments['a.cart-contents'] = ob_get_clean();

		return $fragments;
	}
}
add_filter( 'woocommerce_add_to_cart_fragments', 'cenote_woocommerce_cart_link_fragment' );

if ( ! function_exists( 'cenote_woocommerce_cart_link' ) ) {
	/**
	 * Cart Link.
	 *
	 ...
	 * @return void
	 */
	function cenote_woocommerce_cart_link() {
		?>
			<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'cenote' ); ?>">
				<?php /* translators: number of items in the mini cart. */ ?>
				<span class="amount"><?php echo wp_kses_data( WC()->cart->get_cart_subtotal() ); ?></span> <span class="count"><?php echo wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'cenote' ), WC()->cart->get_cart_contents_count() ) ); ?></span>
			</a>
		<?php
	}
}
...
if ( ! function_exists( 'cenote_woocommerce_header_cart' ) ) {
	 ...
	 * @return void
	 */
	function cenote_woocommerce_header_cart() {
		if ( is_cart() ) {
		...
		<ul id="site-header-cart" class="site-header-cart">
			<li class="<?php echo esc_attr( $class ); ?>">
				<?php cenote_woocommerce_cart_link(); ?>
			</li>
			<li>
				<?php
					$instance = array(
