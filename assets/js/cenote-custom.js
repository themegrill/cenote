	...
	(function addAnimationToMasonry() {
		...
		if (masonryItem) {
			for (index = 0; index < masonryItem.length; index++) {
				masonryItem[index].style.animationDelay =
					2 * (index + 1) + "00ms";
			}
		}
	})();
	...
	(function setupMobileMenu() {
		...
		function closeMobileMenu() {
			...
			if (stickyToggleButton) {
				stickyToggleButton.classList.remove(
		...
		function openMobileMenu() {
			...
			if (stickyToggleButton) {
				stickyToggleButton.classList.add(
					"tg-mobile-menu-toggle--opened"
				);
			}
		...
		if (toggleButton && mobileMenu) {
			toggleButton.addEventListener("click", function () {
				...
				if (stickyToggleButton) {
					stickyToggleButton.classList.toggle(
						"tg-mobile-menu-toggle--opened"
					);
				}
		...
		// 					".tg-post-slider, .post-format-media--gallery",
		// 				)
		// 			) {
		// 				return;
		// 			}
...
(function () {
	...
	(function () {
		...
		if ("ontouchstart" in window) {
			...
			submenuParent = document.querySelectorAll(
				".main-navigation .menu-item-has-children, .main-navigation .page_item_has_children, .cenote-mobile-navigation .menu-item-has-children, .cenote-mobile-navigation .page_item_has_children"
			);
	...
	(function setupStickyHeader() {
		var stickyHeader = document.querySelector(".cenote-header-sticky"),
			offsetHeight =
				document.querySelector(".tg-site-header").offsetHeight,
			stickHeader;
	...
	(function visibleOnHover() {
		...
		if (visible) {
			visible.addEventListener("mouseover", function () {
				document
					.getElementById("cenote-sticky-header")
